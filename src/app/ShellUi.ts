...
export type ShellScreen = 'loading' | 'menu' | 'pause' | 'settings' | 'howto' | 'results' | null;
...
export class ShellUi {
  ...
  constructor() {
    for (const id of ['loading', 'menu', 'pause', 'settings', 'howto', 'results', 'rotate']) {
  ...
  showResults(stats: RunStats, bestTimeMs: number | null): void {
    const list = document.getElementById('results-stats');
    if (!list) return;
    const rows: [string, string, boolean][] = [
      ['Время прохождения', formatTime(stats.timeMs), true],
      ['Лучшее время', bestTimeMs !== null ? formatTime(bestTimeMs) : '—', false],
